...
<script>
	...
	function edit(editBtn){
		...
		var arr = rStr.split(",");
		location.href="${contextPath}/mg/bookUpdate.do?book_id=" + arr[0] + "&book_kind=" + arr[1];
	}
	
	function del(delBtn){
		var rStr = delBtn.name;
		var arr = rStr.split(",");
		window.location.href="${contextPath}/mg/bookDeletePro.do?book_id=" + arr[0] + "&book_kind=" + arr[1];
	}
</script>
...
<div id="books">
  ...
  <table> 
   ...
   <tr>
      ...
      <td width="50"><button id="edit" name="${book.getBook_id()},${book.getBook_kind()}" onclick="edit(this)">수정</button></td>
	  <td width="50"><button id="delete" name="${book.getBook_id()},${book.getBook_kind()}" onclick="del(this)">삭제</button></td> 
   </tr>
  </c:forEach>
</table>
</c:if>
